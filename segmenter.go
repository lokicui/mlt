...
func segment(query string, useEntity bool, client *wenwen_seg.SegServiceClient) (words []WordInfo, err error) {
	...
	for i := 0; i < len(terms); i++ {
		...
		if ok {
			termstr := resp.QueryGbkSbc[resp.Terms[i].Pos*2 : resp.Terms[e].Pos*2+resp.Terms[e].Len*2]
			word.IsEntity = true
			word.Word, err = iconv.ConvertString(termstr, "gbk", "utf-8")
			i = int(e)
		} else {
			if hasSecond(termid) && i+1 < len(resp.GetTerms()) {
				termstr := resp.QueryGbkSbc[resp.Terms[i].Pos*2 : resp.Terms[i+1].Pos*2+resp.Terms[i+1].Len*2]
				word.Word, err = iconv.ConvertString(termstr, "gbk", "utf-8")
				if err != nil {
					continue
				}
				...
				termstr := resp.QueryGbkSbc[resp.Terms[i].Pos*2 : resp.Terms[i].Pos*2+resp.Terms[i].Len*2]
				word.Word, err = iconv.ConvertString(termstr, "gbk", "utf-8")
				if err != nil {
					continue
				}
